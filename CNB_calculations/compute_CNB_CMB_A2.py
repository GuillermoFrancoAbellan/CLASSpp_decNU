"""
compute_CNB_CMB_A2.py

Scenario A2: single decay channel, solar mass gap, normal ordering.
...
# ─── scenario parameters ───────────────────────────────────────────────────────
m_nuL      = 0.03
idx_n      = 2    # index_ncdm_to_print: daughter ν1 (same for stable and decay)

Gamma_list  = [979.49, 293.85, 97.95]   # [km/s/Mpc]
max_q_list  = [15,     15,     15   ]   # maximum_q_ncdm_decay_dr (solar gap → small)
n_bins_list = [35,     20,     20   ]   # N_momentum_bins_ncdm_decay_dr per Gamma
...
def run_stable():
    """Stable A2: all 3 neutrinos as standard NCDM, NO."""
    p = dict(COMMON)
    p.update({
...
def run_decay(Gamma, max_q, n_bins):
    """Decaying A2: ν3 standard (free-streaming), ν2 → ν1 + ϕ (solar gap)."""
    p = dict(COMMON)
    p.update({
        'N_ncdm_decay_dr'                   : 2,
...
# ─── run CLASS or load ─────────────────────────────────────────────────────────
if COMPUTE_SPECTRA:
    print("Running CLASS for stable A2...")
    Cl_stable, Cl_TT_stable = run_stable()
    np.save('./Cl_avg/Cl_avg_A2_stable_mL0p03.npy', Cl_stable)
    np.save('./Cl_CMB/Cl_TT_A2_stable_mL0p03.npy',  Cl_TT_stable)
...
# ─── load ──────────────────────────────────────────────────────────────────────
Cl_stable    = np.load('./Cl_avg/Cl_avg_A2_stable_mL0p03.npy')
Cl_TT_stable = np.load('./Cl_CMB/Cl_TT_A2_stable_mL0p03.npy')
...
ax.set_title(r'{\bf CMB} ($m_{\nu l}=%.2f\,{\rm eV}$, solar gap, NO)' % m_nuL, pad=10, fontsize=23)
...
fig.tight_layout()
fig.savefig('plots_residuals/CMB_residuals_A2.pdf')
plt.show()
...
leg2.get_frame().set_linewidth(0.5)
fig.tight_layout()
fig.savefig('plots_residuals/CNB_residuals_A2.pdf')
plt.show()
