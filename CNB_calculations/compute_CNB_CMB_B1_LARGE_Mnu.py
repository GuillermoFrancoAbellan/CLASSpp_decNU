"""
compute_CNB_CMB_B1_LARGE_Mnu.py

Scenario B1: two decay channels, atmospheric mass gap, normal ordering.
Same as compute_CNB_CMB_B1.py but for a larger lightest-neutrino mass, m_nuL = 0.2 eV.
...
# ─── scenario parameters ───────────────────────────────────────────────────────
m_nuL      = 0.2                          # lightest neutrino mass [eV]
idx_n      = 1                            # index_ncdm_to_print (daughter species)

# Gamma values and corresponding maximum_q from plot_CNB_CMB_spectra_B1_LARGE_Mnu.ipynb
Gamma_list = [979.49, 489.75, 244.87]     # [km/s/Mpc]
max_q_list = [15,     25,     40]         # maximum_q_ncdm_decay_dr per Gamma
...
if COMPUTE_SPECTRA:
    ...
    np.save('./Cl_avg/Cl_avg_B1_stable_mL0p2.npy', Cl_stable)
    np.save('./Cl_CMB/Cl_TT_B1_stable_mL0p2.npy',  Cl_TT_stable)

    for Gamma, max_q in zip(Gamma_list, max_q_list):
        print(f"Running CLASS for Gamma = {Gamma} km/s/Mpc "
              f"(max_q = {max_q}, adjust_q_binning = no)...")
        Cl_dec, Cl_TT_dec = run_decay(Gamma, max_q)
        tag = gamma_tag(Gamma)
        np.save(f'./Cl_avg/Cl_avg_{tag}.npy', Cl_dec)
        np.save(f'./Cl_CMB/Cl_TT_{tag}.npy',  Cl_TT_dec)

# ─── load all spectra ──────────────────────────────────────────────────────────
Cl_stable    = np.load('./Cl_avg/Cl_avg_B1_stable_mL0p2.npy')
Cl_TT_stable = np.load('./Cl_CMB/Cl_TT_B1_stable_mL0p2.npy')
...
ax.set_title(r'{\bf CMB} ($m_{\nu l}=%.1f\,{\rm eV}$, atmos.\ gap, NO)' % m_nuL, pad=10, fontsize=23)
...
fig.tight_layout()
fig.savefig('plots_residuals/CMB_residuals_B1_LARGE_Mnu.pdf')
plt.show()
...
leg2.get_frame().set_linewidth(0.5)
fig.tight_layout()
fig.savefig('plots_residuals/CNB_residuals_B1_LARGE_Mnu.pdf')
plt.show()
