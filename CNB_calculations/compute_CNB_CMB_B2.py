"""
compute_CNB_CMB_B2.py

Scenario B2: two decay channels, atmospheric mass gap, inverted ordering.
...
# ─── scenario parameters ───────────────────────────────────────────────────────
m_nuL      = 0.02
idx_n      = 1    # index_ncdm_to_print: daughter ν3 (same for stable and decay)

Gamma_list = [1960.78, 980.39, 490.19]   # [km/s/Mpc]
...
def run_stable():
    """Stable B2: 2 standard NCDM species (degenerate ν1+ν2 and ν3)."""
    p = dict(COMMON)
    p.update({
        'neutrino_hierarchy'               : 'inverted',
        'N_ncdm_standard'                  : 2,
        'deg_ncdm_standard'                : '2.0, 1.0',
    ...
    Cl_cnb = compute_power_spectrum_new(extract_delta_new(cosmo, idx_n))
    Cl_TT  = cosmo.lensed_cl(2500)['tt']
    cosmo.struct_cleanup(); cosmo.empty()
    return Cl_cnb, Cl_TT

def run_decay(Gamma, max_q, adjust_q='no'):
    """Decaying B2: ν1+ν2 (degenerate parent) → ν3 (daughter) + ϕ."""
    p = dict(COMMON)
    p.update({
        'N_ncdm_decay_dr'                   : 2,
...
# ─── run CLASS or load ─────────────────────────────────────────────────────────
if COMPUTE_SPECTRA:
    print("Running CLASS for stable B2...")
    Cl_stable, Cl_TT_stable = run_stable()
    np.save('./Cl_avg/Cl_avg_B2_stable_mL0p02.npy', Cl_stable)
    np.save('./Cl_CMB/Cl_TT_B2_stable_mL0p02.npy',  Cl_TT_stable)

    for Gamma, max_q in zip(Gamma_list, max_q_list):
        # g500 (Gamma=490.19) uses adjust_q_binning=yes; others fixed
        adjust_q = 'yes' if Gamma == 490.19 else 'no'
...
# ─── load ──────────────────────────────────────────────────────────────────────
Cl_stable    = np.load('./Cl_avg/Cl_avg_B2_stable_mL0p02.npy')
Cl_TT_stable = np.load('./Cl_CMB/Cl_TT_B2_stable_mL0p02.npy')
...
ax.set_title(r'{\bf CMB} ($m_{\nu l}=%.2f\,{\rm eV}$, atmos.\ gap, IO)' % m_nuL, pad=10, fontsize=23)
...
leg2 = ax.legend([Line2D([0],[0], color='lightgray', linewidth=4)],
                 [r'\textsl{Planck} errors'], loc=(0.36, 0.05),
                 frameon=True, facecolor='white', edgecolor='black', framealpha=1.0)
leg2.get_frame().set_linewidth(0.5)
fig.tight_layout()
fig.savefig('plots_residuals/CMB_residuals_B2.pdf')
plt.show()
...
leg2.get_frame().set_linewidth(0.5)
fig.tight_layout()
fig.savefig('plots_residuals/CNB_residuals_B2.pdf')
plt.show()
