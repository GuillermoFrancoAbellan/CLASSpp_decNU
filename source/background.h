...
// GFA, list of possible neutrino mass hierarchy
enum neutrino_hierarchy {normal,inverted,none};

// GFA, list of possible mass gaps for the NCDM->NCDM+DR decay
enum decay_mass_gap {atmospheric,solar};
...
struct background
{
  ...
  enum neutrino_hierarchy nu_hierarchy; /**< GFA, neutrino mass hierarchy */

  enum decay_mass_gap nu_mass_gap_decay; /**< GFA, neutrino mass hierarchy */
  ...
  double Mnu_tot; // GFA, total neutrino mass in eV
  double a_NR; // GFA, scale factor of non-relat. transition for the parent neutrino in DNCDM->NCDM+DR
  double a_decay2; // GFA, approx. scale factor at which parent neutrino population has decayed entirely in DNCDM->NCDM+DR
  int Nq_interp; // GFA, number of q-bins used for integration of collision integral (in DNCDM->NCDM+DR) when interpolating the PSD
  int N_decay_dr = 0;
  ...
  short has_ncdm_decay_dr_ncdm; /**< presence of decaying cold dark matter with massive decay product? */ //NT
  short is_ncdm_decay_degenerate; // GFA, to decide whether the NCDM decay product in NCDM->DR+NCDM is degenerate or not
  short adjust_q_binning; // GFA, to decide whether we adjust the q-binning (i.e. N_q and q_max) in NCDM->DR+NCDM
                          // to the "optimal" choices given the values of Gamma, neutrino masses, etc.
  short has_negative_nu_mass; //GFA, to decide whether we allow negative values for the neutrino mass        
  ...
  int index_ncdm_to_print; // GFA, to decide if we want to print
  //@}
