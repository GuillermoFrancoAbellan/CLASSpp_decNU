...
int InputModule::FixUnknownParameters(int input_verbose, int unknown_parameters_size, int* target_indices) {
  ...
  for (int counter = 0; counter < unknown_parameters_size; counter++){
    ...
    // GFA, when we consider decays NCDM->DR+NCDM, we only want to do shooting for the first NCDM species, so we need to substract 1 to params_size
    if ((strstr(kTargetNamestrings_[index_target].c_str(),"dncdm") != NULL) || (strstr(kTargetNamestrings_[index_target].c_str(),"ncdm_decay") != NULL)) {
...
int InputModule::input_init() {
  ...
  // GFA, this flag needs to be read here, because it affects the shooting method
  pba->has_ncdm_decay_dr_ncdm = _FALSE_;
  class_call(parser_read_string(pfc, "has_ncdm_decay_dr_ncdm", &string1, &flag1, errmsg), errmsg, errmsg);

   if (flag1 == _TRUE_) {
     if((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)){
         pba->has_ncdm_decay_dr_ncdm = _TRUE_;
       } else {
         pba->has_ncdm_decay_dr_ncdm = _FALSE_;
       }
   }
...
// GFA, we add ur contribution later
//  Omega_tot += pba->Omega0_ur;
  ...
  class_call(parser_read_string(pfc, "has_negative_nu_mass", &string1, &flag1, errmsg), errmsg, errmsg);
  // GFA, read flag to decide whether we allow for negative neutrino masses
   if (flag1 == _TRUE_) {
     if((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)){
         pba->has_negative_nu_mass = _TRUE_;
       } else {
         pba->has_negative_nu_mass = _FALSE_;
       }
   }
   class_test(((pba->has_negative_nu_mass == _TRUE_) && ((pba->has_ncdm_decay_dr == _TRUE_) || (pba->has_ncdm_decay_dr_ncdm == _TRUE_))), //GFA
               errmsg,
               "Negative neutrino masses are not yet coded for the decay cases NCDM->DR or NCDM->NCDM+DR \n");

   class_test(((pba->has_negative_nu_mass == _TRUE_) && (ppr->ncdm_fluid_approximation != ncdmfa_none)), //GFA
                errmsg,
                "If you use negative neutrino masses, then you need to desactivate the fluid approximation \n");

  class_call(parser_read_string(pfc,"neutrino_hierarchy",&string1,&flag1,errmsg), //GFA, read neutrino mass hierarchy
             errmsg,
             errmsg);
  if (flag1 == _TRUE_) {
    if ((strstr(string1,"normal") != NULL) || (strstr(string1,"NORMAL") != NULL) || (strstr(string1,"Normal") != NULL)) {
      pba->nu_hierarchy = normal;
    }
    else if ((strstr(string1,"inverted") != NULL) || (strstr(string1,"INVERTED") != NULL) || (strstr(string1,"Inverted") != NULL)) {
      pba->nu_hierarchy = inverted;
    }
    else {
      pba->nu_hierarchy = none;
    }
  }
  ...
  if (pba->has_ncdm_decay_dr_ncdm == _TRUE_) {
    ...
    class_call(parser_read_string(pfc, "is_ncdm_decay_degenerate", &string1, &flag1, errmsg), errmsg, errmsg);
    // GFA, read flag to decide whether we consider the NCDM decay product in NCDM->DR+NCDM to be degenerate or not
     if (flag1 == _TRUE_) {
       if((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)){
           pba->is_ncdm_decay_degenerate = _TRUE_;
         } else {
           pba->is_ncdm_decay_degenerate = _FALSE_;
         }
     }

     class_call(parser_read_string(pfc, "adjust_q_binning", &string1, &flag1, errmsg), errmsg, errmsg);
     // GFA, read flag to decide whether we adjust the q-binning (i.e. N_q and q_max) to the "optimal" choice
      if (flag1 == _TRUE_) {
        if((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)){
            pba->adjust_q_binning = _TRUE_;
          } else {
            pba->adjust_q_binning = _FALSE_;
          }
      }

    class_call(parser_read_string(pfc,"decay_mass_gap",&string1,&flag1,errmsg), //GFA, read decay mass gap
                errmsg,
                errmsg);
     if (flag1 == _TRUE_) {
       if ((strstr(string1,"atmospheric") != NULL) || (strstr(string1,"ATMOSPHERIC") != NULL) || (strstr(string1,"Atmospheric") != NULL)) {
         pba->nu_mass_gap_decay = atmospheric;
       }
       else if ((strstr(string1,"solar") != NULL) || (strstr(string1,"SOLAR") != NULL) || (strstr(string1,"Solar") != NULL)) {
         pba->nu_mass_gap_decay = solar;
       }
       else {
         class_stop(errmsg,"incomprehensible input '%s' for the field 'decay_mass_gap'",string1);
       }
     }

     //GFA
     class_test(((flag1  == _TRUE_) && (pba->nu_hierarchy == none)),
                 errmsg,
                 "You specified a decay mass gap, but you didn't specify a neutrino mass hierarchy (solar or atmospheric) \n");

     class_test(((pba->is_ncdm_decay_degenerate == _TRUE_) && (pba->nu_mass_gap_decay != atmospheric)),
                 errmsg,
                 "In the case of NCDM degenerate decay, you can only set the decay mass gap to atmospheric \n");
  }

  /** - non-cold relics (ncdm) */
  NcdmSettings ncdm_settings;
  ncdm_settings.h = pba->h;
  ncdm_settings.Omega_m = pba->Omega0_b+pba->Omega0_cdm; //GFA
  ...
  ncdm_settings.has_ncdm_decay_dr_ncdm = pba->has_ncdm_decay_dr_ncdm; //GFA
  ncdm_settings.nu_hierarchy = pba->nu_hierarchy; //GFA
  ncdm_settings.is_ncdm_decay_degenerate = pba->is_ncdm_decay_degenerate; //GFA
  ncdm_settings.nu_mass_gap_decay = pba->nu_mass_gap_decay; //GFA
  ncdm_settings.adjust_q_binning = pba->adjust_q_binning; //GFA
  ncdm_settings.has_negative_nu_mass = pba->has_negative_nu_mass; //GFA

  ncdm_ = NonColdDarkMatter::Create(pfc, ncdm_settings);
  if (ncdm_ != nullptr) {
    pba->N_ncdm = ncdm_->N_ncdm_;

    if ((pba->has_ncdm_decay_dr_ncdm == _TRUE_) && (pba->adjust_q_binning ==_TRUE_) && (pba->nu_mass_gap_decay == solar)) { //GFA
      pba->Nq_interp = ncdm_->Nq_interp;
    }
    ...
    if (ncdm_->N_ncdm_decay_dr_ > 0) {
      ...
      int int1, flag4, flag5, flag6, flag7, flag8, temp_size; // temp_size will always be N_ncdm_decay_dr_, the sizes are checked elsewhere  (I added flag7 and flag8, maybe int1) //NT
      char string1[_ARGUMENT_LENGTH_MAX_];
      ...
      if (flag3 == _TRUE_) {
        ...
        for (int n = 0; n < ncdm_->N_ncdm_decay_dr_; n++) {
          ...
          ncdm_->SetDegAndFactor(ncdm_->N_ncdm_standard_ + n, deg_list[n], pba->T_cmb,pba->has_ncdm_decay_dr_ncdm);
        }
        ...
        for (int n = 0; n < ncdm_->N_ncdm_decay_dr_; n++) {
          ncdm_->SetDeg_from_Omega_ini(ncdm_->N_ncdm_standard_ + n, z_ini, pba->H0, Omega_ini_dncdm_list[n], pba->T_cmb,pba->has_ncdm_decay_dr_ncdm);
        }
        ...
        double z_ini = 1.0/a_ini - 1.0;
        for (int n = 0; n < ncdm_->N_ncdm_decay_dr_; n++) {
          ncdm_->SetDeg_from_Omega_ini(ncdm_->N_ncdm_standard_ + n, z_ini, pba->H0, omega_ini_dncdm_list[n]/pba->h/pba->h, pba->T_cmb,pba->has_ncdm_decay_dr_ncdm);
        }
        ...
        for (int n = 0; n < ncdm_->N_ncdm_decay_dr_; n++) {
          ...
          ncdm_->SetDeg_from_Omega_ini(ncdm_->N_ncdm_standard_ + n, z_ini, pba->H0, Omega_ini_dncdm, pba->T_cmb,pba->has_ncdm_decay_dr_ncdm);
        }
      }
    }
  ...
  class_read_int("index_ncdm_to_print", pba->index_ncdm_to_print); //GFA, read index of NCDM species whose temperature multipoles
                                                                  // we want to print, this can go from 0 to N_ncdm -1
  /** - Dark radiation */
...
//   pba->N_decay_dr = 1;
   pba->N_decay_dr = dr_sources.size()-1; // GFA
 } else {
   dr_ = DarkRadiation::Create(pfc, dr_sources, dr_types, dr_deg, pba->T_cmb);
 ...
 if (pba->N_ncdm > 0){
   //GFA, compute total neutrino mass in eV
   double mass_nu = 0;
   for (int n = 0; n < pba->N_ncdm; n++){
     mass_nu += ncdm_->GetDeg(n)*ncdm_->GetMassInElectronvolt(n);
   }
   pba->Mnu_tot = mass_nu;

   // GFA, compute a_NR and a_decay, relevant for DNCDM->NCDM+DR
   if (pba->has_ncdm_decay_dr_ncdm == _TRUE_) {
     double deg_factor = 1., Gamma_rescaled, threshold = 9.;
     if ((pba->is_ncdm_decay_degenerate == _TRUE_) && (pba->nu_hierarchy == normal)) {
       deg_factor =2.;
     }
     Gamma_rescaled = deg_factor*ncdm_->decay_dr_map_[ncdm_->N_ncdm_standard_+0].Gamma/pba->H0;
     if (Gamma_rescaled>threshold) {
       pba->a_decay2 = pow(1.+(pow(Gamma_rescaled/threshold,2.)-1.)/(pba->Omega0_cdm+pba->Omega0_b),-1./3.);}
     else {
       pba->a_decay2 = 1.;
     }
     pba->a_NR = 3.15/ncdm_->M_ncdm_[ncdm_->N_ncdm_standard_+0];
   }
...
//  pba->has_ncdm_decay_dr_ncdm = _FALSE_; //NT
  pba->has_dr = _FALSE_;
  pba->has_scf = _FALSE_;
...
int InputModule::input_default_params() {
  ...
  pba->index_ncdm_to_print = 0; //GFA
  pba->is_ncdm_decay_degenerate = _FALSE_; //GFA
  pba->adjust_q_binning = _FALSE_; //GFA
  pba->has_negative_nu_mass = _FALSE_; //GFA
  pba->Mnu_tot = 0.; //GFA
  pba->a_NR = 1.e-4; //GFA
  pba->a_decay2 = 1.; //GFA
  pba->Nq_interp = 50; //GFA
  pba->nu_hierarchy = none;//GFA
  pba->nu_mass_gap_decay = atmospheric;//GFA
  ...
  for (int counter = 0; counter < pfzw->unknown_parameters_size; ++counter) {
    ...
    case Omega_ini_dncdm: {
      ...
      for (const auto& [ncdm_id, dncdm_properties] : bam->ncdm_->decay_dr_map_) {
        ...
        if (ba.has_ncdm_decay_dr_ncdm == _TRUE_) { //GFA
          if (dncdm_properties.dncdm_id == 0) {
            ...
            double rho_dncdm2_today = bam->background_table_[(bam->bt_size_ - 1)*bam->bg_size_ + bam->index_bg_rho_ncdm1_ + ncdm_id+1];

            if (input_verbose > 0) {
              if ((pfzw->target_name[counter] == omega_dncdmdr) || (pfzw->target_name[counter] == Omega_dncdmdr)) {
                printf(" -> Shooting iteration: Omega_dncdmdrncdm (input) = %g, Omega_dncdmdrncdm (computed) =  %g \n",bam->ncdm_->Omega_dncdmdr_[dncdm_properties.dncdm_id],(rho_dr_today+rho_dncdm_today+rho_dncdm2_today)/ba.H0/ba.H0);
              }
              else {
                printf(" -> Shooting iteration; deg_ncdm_decay_dr_ncdm (input) = %g, Omega_dncdmdrncdm (computed) = %g,  Omega_dncdmdrncdm (input) = %g \n", bam->ncdm_->GetDeg(ncdm_id), (rho_dr_today+rho_dncdm_today+rho_dncdm2_today)/ba.H0/ba.H0, bam->ncdm_->Omega_dncdmdr_[dncdm_properties.dncdm_id]);
              }
            }

            output[idx + dncdm_properties.dncdm_id] = (rho_dr_today + rho_dncdm_today + rho_dncdm2_today)/ba.H0/ba.H0 - bam->ncdm_->Omega_dncdmdr_[dncdm_properties.dncdm_id];
          }
  ...
  for (int counter = 0; counter < pfzw->unknown_parameters_size; counter++) {
    ...
    case Omega_dncdmdr: {
      ...
      for (const auto& [ncdm_id, dncdm_properties] : ncdm->decay_dr_map_) {
        ...
        if (ba.has_ncdm_decay_dr_ncdm == _TRUE_) { //GFA, improved analytical guess to take into account contribution from NCDM daughter
          if (dncdm_properties.dncdm_id == 0) {
            double M_1 = ncdm->M_ncdm_[ncdm_id];
            double M_2 = ncdm->M_ncdm_[ncdm_id+1];
            double eps = 1./2.*(1.-(M_2*M_2)/(M_1*M_1));
            double deg_ncdm2 = ncdm->GetDeg(ncdm_id+1);
            double rho_dncdm2;
            ncdm->background_ncdm_momenta_deg(ncdm_id, deg_ncdm2, z_ini, ba.T_cmb, NULL, &rho_dncdm2, NULL, NULL, NULL);
            double Omega_ini_ncdm_daughter = rho_dncdm2*pow(a_ini, 4.)/ba.H0/ba.H0;
            ...
            if (dncdm_properties.Gamma/ba.H0 > 1.0) {
              ...
              Omega_ini = pow((eps*k_rad*(2*sqrt(x)+sqrt(_PI_)*experfcsqrtx)/(sqrt(2.)*a_nr*sqrt(dncdm_properties.Gamma))) + pow(a_nr,-1.)*(1.-eps),-1.)*(Omega0_dncdmdr_target-pow(a_nr,-1.)*Omega_ini_ncdm_daughter);
              double Omega_ini2 = pow((eps*k_rad*(2*sqrt(x)+sqrt(_PI_)*experfcsqrtx)/(sqrt(2.)*a_nr*sqrt(dncdm_properties.Gamma))) + pow(a_nr,-1.)*(1.-eps),-1.)*Omega0_dncdmdr_target;
              xguess[index_guess_local] = Omega_ini/Omega_deg1;
              dxdy[index_guess_local] = Omega_ini2/Omega_deg1/pfzw->target_values[index_guess_local];

            } else {
              xguess[index_guess_local] = Omega_ini/Omega_deg1;
              dxdy[index_guess_local] = Omega_ini/Omega_deg1/pfzw->target_values[index_guess_local];
            }
      ...
      for (const auto& [ncdm_id, dncdm_properties] : ncdm->decay_dr_map_) {
        ...
        if (ba.has_ncdm_decay_dr_ncdm == _TRUE_) { //GFA, improved analytical guess to take into account contribution from NCDM daughter
          if (dncdm_properties.dncdm_id == 0) {
            ...
            double a_ini = ncdm->GetIni(pr.a_ini_over_a_today_default*ba.a_today, ba.a_today, pr.tol_ncdm_initial_w);
            double z_ini = 1.0/a_ini - 1.0;
            double rho_dncdm, rho_dncdm2;
            ncdm->background_ncdm_momenta_deg(ncdm_id, deg_ncdm2, z_ini, ba.T_cmb, NULL, &rho_dncdm2, NULL, NULL, NULL);
            double Omega_or_omega_ini_ncdm_daughter = rho_dncdm2*pow(a_ini, 4.)/ba.H0/ba.H0;
            ...
            if (dncdm_properties.Gamma/ba.H0 > 1.0) {
              ...
              double x = dncdm_properties.Gamma*t_nr;
              double experfcsqrtx = (x < 20.) ? exp(x)*erfc(sqrt(x)) : 1./sqrt(x*M_PI);
              Omega_or_omega_dncdmdr = eps*(k_rad*Omega_or_omega_ini_dncdm_target*(2*sqrt(x) + sqrt(_PI_)*experfcsqrtx))/(sqrt(2.)*a_nr*sqrt(dncdm_properties.Gamma));
              Omega_or_omega_dncdmdr += pow(a_nr,-1.)*(Omega_or_omega_ini_ncdm_daughter + (1.-eps)*Omega_or_omega_ini_dncdm_target); //GFA, add present contribution from daughter NCDM
              xguess[index_guess_local] = Omega_or_omega_dncdmdr;
              dxdy[index_guess_local] = -(Omega_or_omega_dncdmdr-pow(a_nr,-1.)*Omega_or_omega_ini_ncdm_daughter)/pfzw->target_values[index_guess];
            } else {
              xguess[index_guess_local] = Omega_or_omega_dncdmdr;
              dxdy[index_guess_local] = -Omega_or_omega_dncdmdr/pfzw->target_values[index_guess];
            }
