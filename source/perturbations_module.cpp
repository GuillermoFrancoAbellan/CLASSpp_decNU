...
int PerturbationsModule::perturb_output_titles(enum file_format output_format, char titles[_MAXTITLESTRINGLENGTH_]) const {
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles, "d_dr", pba->has_dr); //NT Do i have to store anything here for massive decay products?
      ...
      if ((pba->has_ncdm == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (has_source_delta_m_ == _TRUE_))) {
        for(n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
          ...
          int idx_n = pba->index_ncdm_to_print; // GFA
           if(ppt->perturbations_verbose > 0){
             for (int index_q=0; index_q < ncdm_->q_size_ncdm_[idx_n]; index_q++) {
               if (index_q==0) {
                  printf(" -> Back. and perts. of NCDM species i=%d are evolved for the following q-bins:\n",idx_n+1);
                  printf("[%g,",ncdm_->q_ncdm_[idx_n][index_q]);
               } else if (index_q==ncdm_->q_size_ncdm_[idx_n]-1) {
                  printf("%g]",ncdm_->q_ncdm_[idx_n][index_q]);
                  printf(" \n");
               } else {
                  printf("%g,",ncdm_->q_ncdm_[idx_n][index_q]);
          ...
          int Nq, Nq_L, Nq_H;
          double dq = ncdm_->q_ncdm_[idx_n][1]-ncdm_->q_ncdm_[idx_n][0];
          double q_thres=15.;
          if ((pba->has_ncdm_decay_dr_ncdm == _TRUE_) && (pba->is_ncdm_decay_degenerate == _TRUE_) && (pba->nu_hierarchy == inverted) && (ncdm_->ncdm_qmax_[idx_n] >= 35.) ) {
            q_thres = 25.;
          }
          if (ncdm_->ncdm_qmax_[idx_n] <= 15.) {
            Nq= MIN(15,ncdm_->q_size_ncdm_[idx_n]);
          } else { // if qmax > 15, then concentrate Nq_L~10 points in range [0,q_thres], and the remaining Nq_H~5 points in the high-q tail
            Nq_L = MIN(10,(int)(q_thres/dq));
            Nq_H = 15 - Nq_L;
            Nq = Nq_L+Nq_H;
          }
            ...
            for (int i=0; i < Nq; i++) {
              if (ncdm_->ncdm_qmax_[idx_n] <= 15.) {
                index_q = (i*(ncdm_->q_size_ncdm_[idx_n]-1))/(Nq-1);
              } else {
                 if (i < Nq_L) {
                   index_q = i*((int)(q_thres/dq)-1)/(Nq_L-1);
                 } else {
                   index_q = (int)(q_thres/dq) + (i-(Nq_L-1))*(ncdm_->q_size_ncdm_[idx_n]-1-(int)(q_thres/dq))/Nq_H;
                 }
              }

              if(ppt->perturbations_verbose > 0){
                if (i==0) {
                   printf(" -> Printing Theta_%d_q_l only for the following q-bins:\n",idx_n+1);
                   printf("[%g,",ncdm_->q_ncdm_[idx_n][index_q]);
                } else if (i==Nq-1) {
                   printf("%g]",ncdm_->q_ncdm_[idx_n][index_q]);
                   printf(" \n");
              ...
              for (index_l=0; index_l<=ppr->l_max_ncdm; index_l++) {
                sprintf(tmp,"Theta_n_q_l_ncdm[%d,%d,%d]",idx_n,index_q,index_l);
                class_store_columntitle(scalar_titles_,tmp,_TRUE_);
              }
            }
            ...
            switch (ncdm_->ncdm_types_[n_ncdm]) {
              case NCDMType::standard:
                for (index_q = 0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q++) {
                  ...
                  if (pba->has_negative_nu_mass == _TRUE_) {
                    epsilon = sgn(ncdm_->M_ncdm_[n_ncdm])*sqrt(q*q + a*a*ncdm_->M_ncdm_[n_ncdm]*ncdm_->M_ncdm_[n_ncdm]);
                  } else {
      ...
      if(ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on){
        ...
        for(n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
          ...
          for (index_q=0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q ++) {
            ...
            q2 = q*q;
            if (pba->has_negative_nu_mass == _TRUE_) {
              epsilon = sgn(ncdm_->M_ncdm_[n_ncdm])*sqrt(q2 + ncdm_->M_ncdm_[n_ncdm]*ncdm_->M_ncdm_[n_ncdm]*a2);
            } else {
...
int PerturbationsModule::perturb_print_variables_member(double tau, double* y, double* dy, void* parameters_and_workspace, ErrorMsg error_message) {
  ...
  double a,a2,H;
  int idx,index_q, storeidx, index_l, i;
  double *dataptr;
  ...
  if (_scalars_) {
      ...
      if(ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on){
        ...
        for(n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
          ...
          for (index_q=0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q ++) {
            ...
            q = ncdm_->q_ncdm_[n_ncdm][index_q];
            q2 = q*q;
            if (pba->has_negative_nu_mass == _TRUE_) { //GFA
               epsilon = sgn(ncdm_->M_ncdm_[n_ncdm])*sqrt(q2 + ncdm_->M_ncdm_[n_ncdm]*ncdm_->M_ncdm_[n_ncdm]*a2);
            } else {
               epsilon = sqrt(q2 + ncdm_->M_ncdm_[n_ncdm]*ncdm_->M_ncdm_[n_ncdm]*a2);
            }
    ...
    if ((pba->has_ncdm == _TRUE_) && ((ppt->has_density_transfers == _TRUE_) || (ppt->has_velocity_transfers == _TRUE_) || (has_source_delta_m_ == _TRUE_))) {
      ...
      if (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on) {
         ...
         // GFA, rather printing Theta_n_q_l for all evolved q-bins (q_size_ncdm_), we print only a subset of 15 q-bins in the range [0,ncdm_qmax_]
         // This is a dirty solution to avoid memory errors that arise when asking to print Theta_n_q_l for more than 15 q-bins
         int Nq, Nq_L, Nq_H;
         double dq = ncdm_->q_ncdm_[idx_n][1]-ncdm_->q_ncdm_[idx_n][0];
         double q_thres = 15.;
         ...
         if (ncdm_->ncdm_qmax_[idx_n] <= 15.) {
          ...
          for (int i=0; i < Nq; i++) {
            ...
            const int idx = ppw->pv->index_ncdm_[idx_n][index_q];
            double dlnf0_dlnq;
            switch (ncdm_->ncdm_types_[idx_n]) {
              case NCDMType::standard: {
                dlnf0_dlnq = ncdm_->dlnf0_dlnq_ncdm_[idx_n][index_q];
                break;
              }
              case NCDMType::decay_dr: {
                // If the current species can decay, make sure to get the time-dependent distribution function
                dlnf0_dlnq = pvecback[background_module_->index_bg_dlnfdlnq_ncdm_decay_dr1_ + ncdm_->decay_dr_map_[idx_n].q_offset + index_q];
                break;
              }
            }
            for (index_l=0; index_l<=ppw->pv->l_max_ncdm[idx_n]; index_l++) {
              class_store_double(dataptr, -y[idx+index_l]/dlnf0_dlnq, _TRUE_, storeidx);
            }
          }
        }
...
int PerturbationsModule::perturb_derivs_member(double tau, double* y, double* dy, void* parameters_and_workspace, ErrorMsg error_message ) {
  ...
  double logf0_nu_dec, psi_nu_dec, frac; //GFA
  int idx_interp, idx_h_0, idx_h_1, idx_h_2, idx_h_3; // GFA
      ...
      if (pba->has_ncdm_decay_dr) {
        for (const auto& id_and_properties : ncdm_->decay_dr_map_) {
          ...
          if (pba->has_ncdm_decay_dr_ncdm == _TRUE_) {
            if ((ncdm_id-ncdm_->N_ncdm_standard_) == 0) { // GFA
                double M_ncdm = ncdm_->M_ncdm_[ncdm_id];
                double M_ncdm2 = ncdm_->M_ncdm_[ncdm_id + 1];
                double eps = 1./2.*(1-(M_ncdm2*M_ncdm2)/(M_ncdm*M_ncdm));
                ...
                double rprime_dr = deg_factor*eps*pow(a,5)/pba->H0/pba->H0*M_ncdm*Gamma*pvecback[background_module_->index_bg_number_ncdm1_ + ncdm_id];
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      /** - ----> second case: use exact equation (Boltzmann hierarchy on momentum grid) */
        ...
        for (n_ncdm=0; n_ncdm<pv->N_ncdm; n_ncdm++) {
          ...
          std::vector<double> Cl((pv->l_max_ncdm[n_ncdm])*(pv->q_size_ncdm[n_ncdm]), 0.0); //NT
          ...
          for (index_q=0; index_q < pv->q_size_ncdm[n_ncdm]; index_q++) {
            ...
            if (pba->has_negative_nu_mass == _TRUE_) {
              epsilon = sgn(ncdm_->M_ncdm_[n_ncdm])*sqrt(q*q + a2*ncdm_->M_ncdm_[n_ncdm]*ncdm_->M_ncdm_[n_ncdm]);
            } else {
              epsilon = sqrt(q*q + a2*ncdm_->M_ncdm_[n_ncdm]*ncdm_->M_ncdm_[n_ncdm]);
            }

            qk_div_epsilon = k*q/epsilon;
            int q_size = pv->q_size_ncdm[n_ncdm]; //NT

            //NT
            if ((pba->has_ncdm_decay_dr_ncdm == _TRUE_) && ((n_ncdm - ncdm_->N_ncdm_standard_) == 1) ) { //GFA
              const DecayDRProperties& dncdm_l = ncdm_->decay_dr_map_[n_ncdm];
              const DecayDRProperties& dncdm_h = ncdm_->decay_dr_map_[n_ncdm - 1];
              double M_l = ncdm_->M_ncdm_[n_ncdm];
              double M_h = ncdm_->M_ncdm_[n_ncdm - 1];
              double f_l = exp(pvecback[background_module_->index_bg_lnf_ncdm_decay_dr1_ + dncdm_l.q_offset + index_q]);
              double deg_factor; // GFA
              ...
              double pref = (a2*deg_factor*dncdm_h.Gamma*M_h*M_h*M_h)/(epsilon*q*f_l*(M_h*M_h - M_l*M_l));
              double q1p = abs((epsilon*(M_h*M_h - M_l*M_l) + q*(M_h*M_h + M_l*M_l))/(2*M_l*M_l));
              double q1m = abs((epsilon*(M_h*M_h - M_l*M_l) - q*(M_h*M_h + M_l*M_l))/(2*M_l*M_l));

              //GFA: Below I implemented the trick of performing the collision integral on a finer grid by interpolating the PSD
              // NOTE: I put it under "FALSE" statement because it seems to be extremelly slow
  //            if ((a>0.1*pba->a_NR) && (a<pba->a_decay2) && (pba->adjust_q_binning == _TRUE_) && (pba->nu_mass_gap_decay== solar)) {
              if (_FALSE_) {
                for (int l =0; l < pv->l_max_ncdm[n_ncdm]; l++) {
                  double integral = 0.;
                  for (int j = 0; j < pba->Nq_interp; j++) {
                    double dq = ncdm_->ncdm_qmax_[n_ncdm - 1]/pba->Nq_interp;
                    double q1 = dq + j*dq;
                    if (j == pba->Nq_interp - 1) {dq *= 0.5;}
                    if ((q1 >= q1m) && (q1 <= q1p)) {
                      double epsilon_h = sqrt(q1*q1 + a2*M_h*M_h);
                      double cos_a = (2*epsilon_h*epsilon - a2*(M_h*M_h + M_l*M_l))/(2*q1*q);
                      ...
                      idx_interp = (int)floor((q1-ncdm_->q_ncdm_[n_ncdm - 1][0])/dncdm_h.dq[10]);
                      if (idx_interp<1) idx_interp=1;
                      if (idx_interp>ncdm_->q_size_ncdm_[n_ncdm - 1]-3) idx_interp=ncdm_->q_size_ncdm_[n_ncdm - 1]-3;
                      frac = ((q1-ncdm_->q_ncdm_[n_ncdm - 1][0])/dncdm_h.dq[10])-idx_interp;
                      const int idx_h_0 = ppw->pv->index_ncdm_[n_ncdm - 1][idx_interp-1];
                      const int idx_h_1 = ppw->pv->index_ncdm_[n_ncdm - 1][idx_interp];
                      const int idx_h_2 = ppw->pv->index_ncdm_[n_ncdm - 1][idx_interp+1];
                      const int idx_h_3 = ppw->pv->index_ncdm_[n_ncdm - 1][idx_interp+2];
                      logf0_nu_dec = pvecback[background_module_->index_bg_lnf_ncdm_decay_dr1_ + dncdm_h.q_offset + idx_interp-1]*frac*(1.-frac)*(2.-frac)/6.
                      +pvecback[background_module_->index_bg_lnf_ncdm_decay_dr1_ + dncdm_h.q_offset + idx_interp]*(1.+frac)*(1.-frac)*(2.-frac)/2.
                      +pvecback[background_module_->index_bg_lnf_ncdm_decay_dr1_ + dncdm_h.q_offset + idx_interp+1]*(1.+frac)*frac*(2.-frac)/2.
                      +pvecback[background_module_->index_bg_lnf_ncdm_decay_dr1_ + dncdm_h.q_offset + idx_interp+2]*(1.+frac)*frac*(frac-1.)/6.;
                      double w = dq*exp(logf0_nu_dec);
                      psi_nu_dec = y[idx_h_0 + l]*frac*(1.-frac)*(2.-frac)/6. + y[idx_h_1 + l]*(1.+frac)*(1.-frac)*(2.-frac)/2.
                      +y[idx_h_2 + l]*(1.+frac)*frac*(2.-frac)/2. + y[idx_h_3 + l]*(1.+frac)*frac*(frac-1.)/6.;
                      integral += w*(q1/epsilon_h)*(psi_nu_dec*Storage_B::Legendre::Pn(l, cos_a) - y[idx + l]);
                    }
                  }
                  Cl[l*q_size + index_q] = pref*integral;
                }
              } else {
                for (int l =0; l < pv->l_max_ncdm[n_ncdm]; l++) {
                  double integral = 0.;
                  for (int index_q_h = 0; index_q_h < pv->q_size_ncdm[n_ncdm - 1]; index_q_h++) {
                    double q_h = ncdm_->q_ncdm_[n_ncdm - 1][index_q_h];
                    const int idx_h = ppw->pv->index_ncdm_[n_ncdm - 1][index_q_h];
                    if ((q_h >= q1m) && (q_h <= q1p)) {
                      double epsilon_h = sqrt(q_h*q_h + a2*M_h*M_h);
                      double dq = dncdm_h.dq[index_q_h];
                      double cos_a = (2*epsilon_h*epsilon - a2*(M_h*M_h + M_l*M_l))/(2*q_h*q);
                      double w = dq*exp(pvecback[background_module_->index_bg_lnf_ncdm_decay_dr1_ + dncdm_h.q_offset + index_q_h]);
                      integral += w*(q_h/epsilon_h)*(y[idx_h + l]*Storage_B::Legendre::Pn(l, cos_a) - y[idx + l]);
                    }
                  }
                  Cl[l*q_size + index_q] = pref*integral;
                }
            ...
            dy[idx] = -qk_div_epsilon*y[idx+1]+metric_continuity*dlnf0_dlnq/3. + Cl[0*q_size + index_q];

            /** - -----> ncdm velocity for given momentum bin */
            dy[idx+1] = qk_div_epsilon/3.0*(y[idx] - 2*s_l[2]*y[idx+2])
              -epsilon*metric_euler/(3*q*k)*dlnf0_dlnq + Cl[1*q_size + index_q];

            /** - -----> ncdm shear for given momentum bin */

            dy[idx+2] = qk_div_epsilon/5.0*(2*s_l[2]*y[idx+1]-3.*s_l[3]*y[idx+3])
              -s_l[2]*metric_shear*2./15.*dlnf0_dlnq + Cl[2*q_size + index_q];

            /** - -----> ncdm l>3 for given momentum bin */

            for(l=3; l<pv->l_max_ncdm[n_ncdm]; l++){
              dy[idx+l] = qk_div_epsilon/(2.*l+1.0)*(l*s_l[l]*y[idx+(l-1)]-(l+1.)*s_l[l+1]*y[idx+(l+1)]) + Cl[l*q_size + index_q];
            }
