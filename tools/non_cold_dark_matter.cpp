...
int NonColdDarkMatter::background_ncdm_init(FileContent* pfc, const NcdmSettings& ncdm_settings) {
  ...
  double Delta_m2_solar = 7.5e-5; //GFA, solar squared mass splitting in eV^2
  double Delta_m2_atmos = 2.5e-3; //GFA, atmospheric squared mass splitting in eV^2 (absolute value)
  ...
  class_test(((ncdm_settings.has_ncdm_decay_dr_ncdm == _TRUE_) && (N_ncdm_decay_dr_ != 2)), //GFA
             errmsg,
             "In input file, you cannot have has_ncdm_decay_dr_ncdm = y if N_ncdm_decay_dr is not set to 2 \n");

  class_test(((N_ncdm_decay_dr_>0) && (ncdm_settings.has_ncdm_decay_dr_ncdm == _FALSE_) && (ncdm_settings.nu_hierarchy != none) ),//GFA
               errmsg,
               "A specific neutrino hierarchy is not yet coded for the decay case NCDM->DR \n");

  class_test(((ncdm_settings.nu_hierarchy != none) && ((N_ncdm_<2)||(N_ncdm_>3)) ), //GFA
              errmsg,
              "It only makes sense to specify a neutrino hierarchy if N_ncdm is equal to 2 or 3\n");

  if (ncdm_settings.nu_hierarchy != none) { //GFA
      class_read_double("m_ncdm_lightest", m_ncdm_lightest);
  }
  ...
  read_list_of_doubles_with_deprecated("omega_ncdm_standard",
    ...
    class_test(((ncdm_settings.nu_hierarchy  != none ) && (N_ncdm_standard_==2) && (deg_ncdm_[0]==1.0) && (deg_ncdm_[1]==1.0)), //GFA
                errmsg,
                "In you choose a specific neutrino hierarchy and set N_ncdm_standard=2, then you need to set one of the two deg_ncdm_standard to 2 \n");

    if ( (ncdm_settings.nu_hierarchy != none) && (ncdm_settings.has_ncdm_decay_dr_ncdm == _FALSE_)) { // GFA, do mass adjustment only if we selected a specific neutrino hierarchy
        if ((N_ncdm_standard_==2) && ((deg_ncdm_[0]==2.0) || (deg_ncdm_[1]==2.0))) { // check if we are assuming two degenerate masses
              m_ncdm_in_eV_[1] =  m_ncdm_lightest;
              m_ncdm_in_eV_[0] = sqrt(pow(m_ncdm_in_eV_[1],2)+Delta_m2_atmos);
        } else {
          if (ncdm_settings.nu_hierarchy == normal) {
            m_ncdm_in_eV_[2] =  m_ncdm_lightest;
            m_ncdm_in_eV_[1] = sqrt(pow(m_ncdm_in_eV_[2],2)+Delta_m2_solar);
            m_ncdm_in_eV_[0] = sqrt(pow(m_ncdm_in_eV_[2],2)+Delta_m2_atmos);
          }
          if (ncdm_settings.nu_hierarchy == inverted) {
            m_ncdm_in_eV_[2] =  m_ncdm_lightest;
            m_ncdm_in_eV_[1] = sqrt(pow(m_ncdm_in_eV_[2],2)+Delta_m2_atmos);
            m_ncdm_in_eV_[0] = sqrt(pow(m_ncdm_in_eV_[2],2)+Delta_m2_atmos+Delta_m2_solar);
          }
        }
    }
  ...
  if (N_ncdm_decay_dr_ > 0) {
    ...
    class_test(((ncdm_settings.is_ncdm_decay_degenerate  == _TRUE_ ) && (deg_ncdm_[N_ncdm_standard_+0]==1.0) && (deg_ncdm_[N_ncdm_standard_+1]==1.0) ), //GFA
                errmsg,
                "In the case of NCDM degenerate decay, you have to set one of the two deg_ncdm_decay_dr to 2 \n");

    if (ncdm_settings.nu_hierarchy != none) { // GFA, do mass adjustment only if we selected a specific neutrino hierarchy
       if (ncdm_settings.is_ncdm_decay_degenerate  == _TRUE_) {
         m_ncdm_in_eV_[N_ncdm_standard_+1] =  m_ncdm_lightest;
         m_ncdm_in_eV_[N_ncdm_standard_+0] = sqrt(pow(m_ncdm_in_eV_[N_ncdm_standard_+1],2)+Delta_m2_atmos);
       } else {
         if ((ncdm_settings.nu_mass_gap_decay == solar) && (ncdm_settings.nu_hierarchy == normal)) {
           m_ncdm_in_eV_[N_ncdm_standard_+1] =  m_ncdm_lightest;
           m_ncdm_in_eV_[N_ncdm_standard_+0] = sqrt(pow(m_ncdm_in_eV_[N_ncdm_standard_+1],2)+Delta_m2_solar);
           m_ncdm_in_eV_[0] = sqrt(pow(m_ncdm_in_eV_[N_ncdm_standard_+1],2)+Delta_m2_atmos);
         }
         if ((ncdm_settings.nu_mass_gap_decay == solar) && (ncdm_settings.nu_hierarchy == inverted)) {
           m_ncdm_in_eV_[0] =  m_ncdm_lightest;
           m_ncdm_in_eV_[N_ncdm_standard_+1] = sqrt(pow(m_ncdm_in_eV_[0],2)+Delta_m2_atmos);
           m_ncdm_in_eV_[N_ncdm_standard_+0] = sqrt(pow(m_ncdm_in_eV_[0],2)+Delta_m2_atmos+Delta_m2_solar);
         }
         if ((ncdm_settings.nu_mass_gap_decay == atmospheric) && (ncdm_settings.nu_hierarchy == normal)) {
           m_ncdm_in_eV_[N_ncdm_standard_+1] =  m_ncdm_lightest;
           m_ncdm_in_eV_[0] = m_ncdm_in_eV_[N_ncdm_standard_+1]; // i.e. here we neglect solar mass gap
           m_ncdm_in_eV_[N_ncdm_standard_+0] = sqrt(pow(m_ncdm_in_eV_[N_ncdm_standard_+1],2)+Delta_m2_atmos);
         }
         if ((ncdm_settings.nu_mass_gap_decay == atmospheric) && (ncdm_settings.nu_hierarchy == inverted)) {
           m_ncdm_in_eV_[N_ncdm_standard_+1] =  m_ncdm_lightest;
           m_ncdm_in_eV_[N_ncdm_standard_+0] = sqrt(pow(m_ncdm_in_eV_[N_ncdm_standard_+1],2)+Delta_m2_atmos);
           m_ncdm_in_eV_[0] = m_ncdm_in_eV_[N_ncdm_standard_+0]; // i.e. here we neglect solar mass gap
         }
       }
    }

    class_test(((ncdm_settings.has_ncdm_decay_dr_ncdm  == _TRUE_) && (m_ncdm_in_eV_[N_ncdm_standard_+0] < m_ncdm_in_eV_[N_ncdm_standard_+1])), //GFA
                 errmsg,
                 "In the decay case NCDM->DR+NCDM, the second ncdm_decay species is assumed to be the decay product, so it has to be lighter than the first ncdm_decay species\n");
  ...
  if (N_ncdm_decay_dr_ > 0) {
    ...
    double Gamma_nu, log10Gamma_nu, lifetime_nu, log10lifetime_nu; //GFA
    int Gamma_read, log10Gamma_read, lifetime_read, log10lifetime_read;
    int size = 0;

    if (ncdm_settings.has_ncdm_decay_dr_ncdm == _TRUE_) {
      class_call(parser_read_double(pfc, "Gamma_nu", &Gamma_nu, &Gamma_read, errmsg), errmsg, errmsg);
      class_call(parser_read_double(pfc, "log10Gamma_nu", &log10Gamma_nu, &log10Gamma_read, errmsg), errmsg, errmsg);
      class_call(parser_read_double(pfc, "lifetime_nu", &lifetime_nu, &lifetime_read, errmsg), errmsg, errmsg);
      class_call(parser_read_double(pfc, "log10lifetime_nu", &log10lifetime_nu, &log10lifetime_read, errmsg), errmsg, errmsg);
      if (class_at_least_two_of_four(Gamma_read, log10Gamma_read, lifetime_read, log10lifetime_read)) {
        throw std::invalid_argument("More than two of the following inputs are given. Choose one! \nGamma_nu, log10Gamma_nu, lifetime_nu, log10lifetime_nu.");
      }
    } else {
    ...
    if (ncdm_settings.has_ncdm_decay_dr_ncdm == _TRUE_) { // GFA, in case of NCDM->DR+NCDM, force Gamma of the two NCDM species to be the same as the first
      if (Gamma_read == _TRUE_) {
        Gamma_input[0] = Gamma_nu;
      }
      else if (log10Gamma_read == _TRUE_) {
        Gamma_input[0] = pow(10., log10Gamma_nu);
      }
      else if (lifetime_read == _TRUE_) {
        Gamma_input[0] = 1./lifetime_nu/(365*24*60*60)*_Mpc_over_m_*1e-3; // Conversion from yr^-1 to km/s/Mpc
      }
      else if (log10lifetime_read == _TRUE_) {
        double lifetime = pow(10., log10lifetime_nu);
        Gamma_input[0] = 1./lifetime/(365*24*60*60)*_Mpc_over_m_*1e-3;
      }
      Gamma_input[1] = Gamma_input[0];
    } else {
      for (int n = 0; n < N_ncdm_decay_dr_; n++) {
        if (Gamma_read == _TRUE_) {
  ...
  if (ncdm_settings.adjust_q_binning == _TRUE_) { // GFA, do q-binning adjustment
    double q_max;
    int Nq;
    get_Nq_and_qmax(Gamma_input[0],m_ncdm_in_eV_[N_ncdm_standard_+1],ncdm_settings,&q_max, &Nq);
    ncdm_qmax_[N_ncdm_standard_+0] = q_max;
    ncdm_qmax_[N_ncdm_standard_+1] = q_max;
    if (ncdm_settings.nu_mass_gap_decay == solar) {
      Nq_interp = Nq;
      ncdm_input_q_size_[N_ncdm_standard_+0] = 20;
      ncdm_input_q_size_[N_ncdm_standard_+1] = 20;
    }
    else if (ncdm_settings.nu_mass_gap_decay == atmospheric) {
      ncdm_input_q_size_[N_ncdm_standard_+0] = Nq;
      ncdm_input_q_size_[N_ncdm_standard_+1] = Nq;
    }
  ...
  for(int k = 0; k < N_ncdm_; k++){
    ...
    if (ncdm_types_[k] == NCDMType::decay_dr) {
      ...
      if ( (ncdm_quadrature_strategy_[k] != 3) && (ncdm_quadrature_strategy_[k] != 4)) { //GFA
        throw std::invalid_argument("DNCDM currently only admits quadrature strategy 3 or 4. Please change your input accordingly.\n");
      }
      else {
  ...
  for (n=0; n < N_ncdm_; n++){
    if (m_ncdm_in_eV_[n] != 0.0){
      ...
      double rho_ncdm;
      class_call(background_ncdm_momenta(n, 0.,ncdm_settings.has_negative_nu_mass, NULL, &rho_ncdm, NULL, NULL, NULL),
                 error_message_,
                 errmsg);
...
int NonColdDarkMatter::background_ncdm_momenta(int n_ncdm, double z, short has_negative_nu_mass, double* n, double* rho, double* p, double* drho_dM, double* pseudo_p) const {
  return background_ncdm_momenta_mass(n_ncdm, M_ncdm_[n_ncdm], z, has_negative_nu_mass, n, rho, p, drho_dM, pseudo_p);
}

int NonColdDarkMatter::background_ncdm_momenta_mass(int n_ncdm, double M, double z, short has_negative_nu_mass, double* n, double* rho, double* p, double* drho_dM, double* pseudo_p) const {
  ...
  for (int index_q = 0; index_q < qsize; index_q++) {
    ...
    /* energy */
    if (has_negative_nu_mass == _TRUE_) { //GFA
      epsilon = sgn(M)*sqrt(q2 + M*M/(1. + z)/(1. + z));
    } else {
...
double NonColdDarkMatter::background_ncdm_M_from_Omega(int n_ncdm, double H0, double Omega0, double tol_M_ncdm) {
  ...
  double rho;
  double n;
  background_ncdm_momenta_mass(n_ncdm, M, 0.,_FALSE_, &n, &rho, NULL, NULL, NULL);
  ...
  for (int iter = 1; iter <= maxiter; iter++){
    ...
    double drhodM;
    background_ncdm_momenta_mass(n_ncdm, M, 0.,_FALSE_, NULL, &rho, NULL, &drhodM, NULL);
...
void NonColdDarkMatter::PrintNeffInfo(short has_ncdm_decay_dr_ncdm, short has_negative_nu_mass) const {
  int filenum = 0;
  for (int n_ncdm = 0; n_ncdm < N_ncdm_; ++n_ncdm) {
    ...
    double rho_ncdm_rel;
    background_ncdm_momenta_mass(n_ncdm, 0., 0.,_FALSE_, NULL, &rho_ncdm_rel, NULL, NULL, NULL);

    if (has_negative_nu_mass == _TRUE_) {
      rho_ncdm_rel *= sgn(m_ncdm_in_eV_[n_ncdm]);
    }
    ...
    /* inform user of the contribution of each species to
      ...
      printf(" -> ncdm species i=%d sampled with Nq = %d bins and q_max = %g. In the relativistic limit it gives Delta N_eff = %g\n",
             n_ncdm + 1,
             q_size_ncdm_bg_[n_ncdm],
             ncdm_qmax_[n_ncdm],
             rho_ncdm_rel/rho_nu_rel_);

      if (ncdm_quadrature_strategy_[n_ncdm] == 4) {
        int Nq_low = _NQLOW_OVER_NQTOT_*q_size_ncdm_bg_[n_ncdm];
        printf(" -> ncdm species i=%d uses piecewise trapezoidal quadrature, with Nq_L = %d bins in the interval [0,%g] \n",
        n_ncdm + 1,
        Nq_low,
        round(_QTHRES_OVER_QMAX_*ncdm_qmax_[n_ncdm]));
      }

// GFA, I commented this part out
...
//           rho_ncdm_rel/rho_nu_rel_);
  ...
  if ((has_ncdm_decay_dr_ncdm == _TRUE_) && (Nq_interp !=0)) {
    printf(" -> Background collision integral computed by interpolating PSD and evaluating it on %d bins \n",Nq_interp);
  }

}

void NonColdDarkMatter::PrintMassInfo(short has_ncdm_decay_dr_ncdm) const {
  if (has_ncdm_decay_dr_ncdm == _TRUE_) {
    for (int n_ncdm = 0; n_ncdm < N_ncdm_standard_; n_ncdm++) {
      printf(" -> non-cold dark matter species with i=%d has m_i = %g eV (so m_i / omega_i =%g eV)\n",
             n_ncdm + 1,
             m_ncdm_in_eV_[n_ncdm],
             m_ncdm_in_eV_[n_ncdm]*deg_ncdm_[n_ncdm]/omega0_ncdm_[n_ncdm]);
    }

    printf(" -> non-cold dark matter parent (with m_dncdm = %g eV) and daughter (with m_ncdm = %g eV) have m_dncdm+ncdm/omega0_dncdm+dr+ncdm =%g eV)\n",
           m_ncdm_in_eV_[N_ncdm_standard_+0],
           m_ncdm_in_eV_[N_ncdm_standard_+1],
           (m_ncdm_in_eV_[N_ncdm_standard_+0]*deg_ncdm_[N_ncdm_standard_+0]+m_ncdm_in_eV_[N_ncdm_standard_+1]*deg_ncdm_[N_ncdm_standard_+1])/omega0_ncdm_[N_ncdm_standard_+0]);
...
double NonColdDarkMatter::GetNeff(double z, short has_ncdm_decay_dr_ncdm, short has_negative_nu_mass) const {
  double Neff = 0;
  for (int n_ncdm = 0; n_ncdm < N_ncdm_; ++n_ncdm) {
    /* call this function to get rho_ncdm */
    double rho_ncdm_rel;
    background_ncdm_momenta_mass(n_ncdm, 0., z, _FALSE_, NULL, &rho_ncdm_rel, NULL, NULL, NULL);
    if (has_negative_nu_mass == _TRUE_) {
      rho_ncdm_rel *= sgn(m_ncdm_in_eV_[n_ncdm]);
    }
...
void NonColdDarkMatter::SetDegAndFactor(int n_ncdm, double deg, double T_cmb, short has_ncdm_decay_dr_ncdm) {
...
void NonColdDarkMatter::SetDeg_from_Omega_ini(int n_ncdm, double z_ini, double H0, double Omega_ini, double T_cmb,short has_ncdm_decay_dr_ncdm) {
  double rho_deg1;
  ...
  deg_ncdm_[n_ncdm] = Omega_ini/Omega_deg1; //Note, this formula might not work in the limit of a very heavy particle that is always cold, i.e. non-relativistic
...
double NonColdDarkMatter::GetIni(double a, double a_today, double tol_ncdm_initial_w) const {
  ...
  for (counter = 0; counter < _MAX_IT_; counter++) {
      ...
      for (int n_ncdm = 0; n_ncdm < N_ncdm_; n_ncdm++) {
        ...
        class_call(background_ncdm_momenta(n_ncdm, a_today/a - 1.0, _FALSE_, NULL, &rho_ncdm, &p_ncdm, NULL, NULL),
                   error_message_,
                   error_message_);
...
//GFA, Function to get optimal values (i.e. smallest that give accurate results)
// of Nq and qmax given various parameters of the neutrino decay

int NonColdDarkMatter::get_Nq_and_qmax(double Gamma, double m_nuL,const NcdmSettings& ncdm_settings,double* q_max, int* Nq) const {

  double Delta_m2, m_nuH, m_nuH_rescaled, m_nuL_rescaled, Gamma_rescaled;
  double a_decay, a_decay2, epsilon, q_prime_decay, q_prime_decay2;
  double pref, v_nuH_decay2, q_width_decay2;
  double q2_avg, eps2_avg_decay, a, b, Delta_q, dq;

  if (ncdm_settings.nu_mass_gap_decay == atmospheric) {
    Delta_m2 = 2.5e-3; //GFA, atmospheric squared mass splitting in eV^2 (absolute value)
  } else {
    Delta_m2 = 7.5e-5; //GFA, solar squared mass splitting in eV^2
  }
  m_nuH = sqrt(pow(m_nuL,2)+Delta_m2);
  m_nuH_rescaled = m_nuH/_k_B_*_eV_/T_ncdm_[N_ncdm_standard_+0]/ncdm_settings.T_cmb;
  m_nuL_rescaled = m_nuL/_k_B_*_eV_/T_ncdm_[N_ncdm_standard_+1]/ncdm_settings.T_cmb;

  if ((ncdm_settings.is_ncdm_decay_degenerate == _TRUE_) && (ncdm_settings.nu_hierarchy == normal)) {
    Gamma_rescaled = 2.*Gamma/(ncdm_settings.h*100.);
  } else {
    Gamma_rescaled = Gamma/(ncdm_settings.h*100.);
  }

  if (Gamma_rescaled > 1) { // This comes from setting Gamma / H(a) = 1, assuming a matter + Lambda universe for H(a) (i.e., neglect back-reaction of decay)
    a_decay = pow(1.+(-1.+pow(Gamma_rescaled,2))/ncdm_settings.Omega_m, -1./3.);
  } else {
    a_decay = 1.;
  }

  if (Gamma_rescaled > 9) { // This comes from setting Gamma / H(a) = 9, assuming a matter + Lambda universe for H(a) (i.e., neglect back-reaction of decay)
    a_decay2 = pow(1.+(-1.+pow(Gamma_rescaled/9.,2))/ncdm_settings.Omega_m, -1./3.);
  } else {
    a_decay2 = 1.;
  }

  epsilon = Delta_m2/(2.*m_nuH*m_nuH);
  ...
  pref = sqrt(15.*_zeta5_/_zeta3_);
  v_nuH_decay2 = pref/(a_decay2*m_nuH_rescaled);

  // Taken from eq. 5.9 in https://arxiv.org/pdf/2011.01502
  q_width_decay2 = a_decay2*((m_nuH*m_nuH+m_nuL*m_nuL)/(2.*m_nuH*m_nuH))*m_nuH_rescaled*v_nuH_decay2;
  *q_max = MAX(15.,q_prime_decay2+q_width_decay2); // if q is not injected above the qmax of the initial FD distib. (i.e. =15), then leave it to that value
  *q_max = round_up_to_multiple_of_5(*q_max);
  ...
  if (ncdm_settings.nu_mass_gap_decay == solar) {
     ...
     q2_avg = 3.15;
     eps2_avg_decay = sqrt(pow(q2_avg,2)+pow(a_decay*m_nuL_rescaled,2));
     a = (eps2_avg_decay/2.)*(-1.+pow(m_nuH/m_nuL,2));
     b = (q2_avg/2.)*(1.+pow(m_nuH/m_nuL,2));
     if (a<b) {
       Delta_q = 2.*a;
     } else {
       Delta_q = 2.*b;
     ...
     dq = 0.25*Delta_q;
     *Nq = MAX(20,(int)round_up_to_multiple_of_5(*q_max/dq));
  }
  else if (ncdm_settings.nu_mass_gap_decay == atmospheric) {
    if (*q_max < 40.) {
      // If qmax is not too large, use standard trapezoidal quadrature
      ncdm_quadrature_strategy_[N_ncdm_standard_+0] = 3;
      ncdm_quadrature_strategy_[N_ncdm_standard_+1] = 3;
      dq = 1.5; // atmos splitting usually does not require very high resolution
      *Nq = MAX(20,(int)round_up_to_multiple_of_5(*q_max/dq)); // To guarantee good precision, Nq should never go below 20
    } else {
      // If qmax is very large, use new quadrature method qm_trapz_piecewise, that accumulates more points in the low-q part
      ncdm_quadrature_strategy_[N_ncdm_standard_+0] = 4;
      ncdm_quadrature_strategy_[N_ncdm_standard_+1] = 4;
      if (*q_max > 110.) {
//	     *q_max -= 10.0;
        dq = 0.9; // this is the dq in the low-q part
      } else {
//        dq = 1.3;
        dq = 1.2; // this is the dq in the low-q part
      }
      *Nq = MAX(20,(int)round_up_to_multiple_of_5(*q_max*_QTHRES_OVER_QMAX_/(dq*_NQLOW_OVER_NQTOT_)));
    }
  }
  return _SUCCESS_;
}

double NonColdDarkMatter::round_up_to_multiple_of_5(double num) const { //GFA
  return std::floor((num + 4) / 5.0) * 5.0;
}

double NonColdDarkMatter::round_down_to_multiple_of_5(double num) const {
    return std::floor(num / 5.0) * 5.0;
}
