...
struct NcdmSettings {
  ...
  double tol_ncdm_bg;
  double tol_M_ncdm;
  short has_ncdm_decay_dr_ncdm; // GFA
  short is_ncdm_decay_degenerate; // GFA
  short adjust_q_binning; // GFA
  short has_negative_nu_mass; // GFA
  enum neutrino_hierarchy nu_hierarchy; //GFA
  enum decay_mass_gap nu_mass_gap_decay; //GFA
};
...
public:
  ...
  ~NonColdDarkMatter();
  int background_ncdm_momenta(int n_ncdm, double z, short has_negative_nu_mass, double* n, double* rho, double* p, double* drho_dM, double* pseudo_p) const;
  ...
  double GetNeff(double z,short has_ncdm_decay_dr_ncdm, short has_negative_nu_mass) const; //GFA
  double GetMassInElectronvolt(int n_ncdm) const;
  void PrintNeffInfo(short has_ncdm_decay_dr_ncdm, short has_negative_nu_mass) const; //GFA
  void PrintMassInfo(short has_ncdm_decay_dr_ncdm) const; //GFA
  void PrintOmegaInfo() const;
  double round_up_to_multiple_of_5(double num) const; //GFA
  double round_down_to_multiple_of_5(double num) const; //GFA

  void SetBackgroundWeight(int n_ncdm, int q_index, double weight);
  void SetOmega0(int n_ncdm, double Omega0, double h);
  void SetDegAndFactor(int n_ncdm, double deg, double T_cmb, short has_ncdm_decay_dr_ncdm);
  void SetDeg_from_Omega_ini(int n_ncdm, double z_ini, double H0, double Omega_ini, double T_cmb, short has_ncdm_decay_dr_ncdm);
  ...
  double m_ncdm_lightest = 0.0; //GFA
  int Nq_interp = 0; //GFA
  double* m_ncdm_in_eV_ = nullptr;
  double* M_ncdm_ = nullptr;
  int* q_size_ncdm_ = nullptr;    /**< Size of the q_ncdm arrays */
  double* ncdm_qmax_ = nullptr;   /**< Vector of maximum value of q */ //GFA (I moved this from private to public)
...
private:
  ...
  int background_ncdm_momenta_mass(int n_ncdm, double M, double z, short has_negative_nu_mass, double* n, double* rho, double* p, double* drho_dM, double* pseudo_p) const;
  int get_Nq_and_qmax(double Gamma, double m_nuL, const NcdmSettings& ncdm_settings,double* q_max, int* Nq) const; //GFA
