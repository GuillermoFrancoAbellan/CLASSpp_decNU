...
int get_qsampling_manual(double* x,
  ...
  case (qm_auto) :
    ...
    return _SUCCESS_;
    case (qm_trapz_piecewise) : //GFA, new quadrature method
                                // trapezoidal rule in two intervals [0,q_thres] & [q_thres, qmax] (the first with a fraction _NQLOW_OVER_NQTOT_ of Nq)
    N_lowq = _NQLOW_OVER_NQTOT_*N;
    N_highq = N - N_lowq;
    q_thres = round(_QTHRES_OVER_QMAX_*qmax);
    for (int i = 0; i < N_lowq; ++i) {
      h_lowq = q_thres/N_lowq;
      x[i] = h_lowq + i*h_lowq;
      (*function)(params_for_function,x[i],&y);
      dq[i] = h_lowq;
      w[i] = y*h_lowq;
    }
    for (int i = 0; i < N_highq; ++i) {
      h_highq = (qmax-q_thres)/N_highq;
      x[N_lowq+i] = q_thres + h_highq + i*h_highq;
      (*function)(params_for_function,x[N_lowq+i],&y);
      dq[N_lowq+i] = h_highq;
      w[N_lowq+i] = y*h_highq;
      if (i == N_highq - 1) {
        dq[N_lowq+i] *= 0.5;
        w[N_lowq+i] *=0.5;
      }
